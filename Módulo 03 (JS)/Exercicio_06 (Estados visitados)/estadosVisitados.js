...
const playQuiz = confirm("Olá viajante!\n"+
...
if(playQuiz){
  ...
  alert("Você conheceu um total de " + answers.known.length + " estados brasileiros!");
  if(answers.known.length > 0){
    alert("São eles:\n" + answers.known.join(", \n") + ".");
  }
  
  alert("E você ainda não visitou " + answers.notKnown.length + " dos estados brasileiros.");
  ...
  if(answers.known.length > 17){
    ...
    alert("Você já visitou mais de um terço dos estados do Brasil!\n"+
      name +" você pode se considerar um explorador de bagagem! Haha!"
    );
  } else {
    alert(name +", você visitou menos de um terço dos estados do Brasil.\n"+
